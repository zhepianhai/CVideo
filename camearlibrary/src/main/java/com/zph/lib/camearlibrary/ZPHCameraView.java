package com.zph.lib.camearlibrary;
...
import com.zph.lib.camearlibrary.util.FileUtil;
import com.zph.lib.camearlibrary.util.ScreenUtils;
import com.zph.lib.camearlibrary.view.CameraView;
 ...
 * @date 2018/5/4
 */
public class ZPHCameraView extends FrameLayout implements CameraInterface.CameraOpenOverCallback, SurfaceHolder
        .Callback, CameraView {
//    private static final String TAG = "JCameraView";
    ...
    //回调监听
    private ZPHCameraListener jCameraLisenter;
    ...
    public ZPHCameraView(Context context) {
        this(context, null);
    }

    public ZPHCameraView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ZPHCameraView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        mContext = context;
        //get AttributeSet
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.ZPHCameraView, defStyleAttr, 0);
        iconSize = a.getDimensionPixelSize(R.styleable.ZPHCameraView_iconSize, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_SP, 35, getResources().getDisplayMetrics()));
        iconMargin = a.getDimensionPixelSize(R.styleable.ZPHCameraView_iconMargin, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_SP, 15, getResources().getDisplayMetrics()));
        iconSrc = a.getResourceId(R.styleable.ZPHCameraView_iconSrc, R.drawable.ic_camera);
        iconLeft = a.getResourceId(R.styleable.ZPHCameraView_iconLeft, 0);
        iconRight = a.getResourceId(R.styleable.ZPHCameraView_iconRight, 0);
        duration = a.getInteger(R.styleable.ZPHCameraView_duration_max, 10 * 1000);       //没设置默认为10s
        a.recycle();
        initData();
        initView();
    }
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        new Thread() {
            @Override
            public void run() {
                CameraInterface.getInstance().doOpenCamera(ZPHCameraView.this);
            }
        }.start();
    }
...
//                    Log.i("CJT", "result = " + (result - firstTouchLength));
    ...
    public void setJCameraLisenter(ZPHCameraListener jCameraLisenter) {
    ...
    public void playVideo(Bitmap firstFrame, final String url) {
        videoUrl = url;
        ZPHCameraView.this.firstFrame = firstFrame;
